...
| `[HC-NO-EXFILTRATION]` | Partial | `desktop/tests/engine/providers.test.ts` — "gives the runtime no ambient context to read"; `desktop/tests/engine/integrations/runner.test.ts` — "never touches an adapter that is turned off", "contacts only the hosts the constitution names"; `desktop/tests/engine/integrations/github.test.ts` — "contacts only api.github.com", "uses read-only requests", "sends only a query and a credential, never user content", "fetches nothing when no repository is in scope", "searches without a repository qualifier once all repositories are opted in", "bounds a first sync to the lookback window"; `desktop/scripts/smoke-packaged.mjs` — "activity is collected only after the user turns it on" | The Copilot SDK's ambient-file defaults are pinned, and with the first real adapter in place the ingress-only half of the exemption is now automated for it: read-only methods, a host that matches the declaration, a query carrying no user content, and no request at all when scope is empty unless the user has explicitly opted into reading everything. Those assertions cover the GitHub adapter specifically — they are not a property enforced across all adapters, so P4 and P6 each owe their own. There is still no dependency policy or process-wide network assertion, so a new outbound call elsewhere in the codebase would pass the suite. |
