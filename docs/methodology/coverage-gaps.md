...
| `[HC-NO-EXFILTRATION]` | Partial | `desktop/tests/engine/providers.test.ts` — "gives the runtime no ambient context to read"; `desktop/tests/engine/integrations/runner.test.ts` — "never touches an adapter that is turned off", "declares no outbound hosts, because nothing here makes a call"; `desktop/scripts/smoke-packaged.mjs` — "activity is collected only after the user turns it on" | The Copilot SDK's ambient-file defaults are pinned, and the consent half of the integrations exemption is now automated: a disabled adapter is provably never called, and the packaged app collects nothing until the user opts in. The rest of the exemption is still uncovered — nothing checks that an adapter uses read-only methods, that its requests carry no user content, or that the hosts it contacts match the ones it declares. Those assertions are cheap only once a real adapter exists (P3), because today the sole adapter is offline. There is still no dependency policy or network assertion, so a new outbound call elsewhere would pass the suite. |
...
| `[HC-OBSERVATION-VS-INFERENCE]` | Automated | `desktop/tests/engine/validation.test.ts` — "preserves observation and inference fields"; `desktop/tests/engine/activity-grounding.test.ts` — "keeps measured activity out of the user's own claims", "keeps signals out of the mentor's evidence lane", "carries measured activity alongside, not inside, the user's claims" | Field separation is enforced, and measured activity is now provably kept out of both `current_state` (what the user claims) and `sources` (what supports the mentor). Whether the model actually put the right content in each field is still not checked. |
