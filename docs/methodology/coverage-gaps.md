...
| --- | --- | --- | --- |
| `[HC-NO-PLAINTEXT-HISTORY]` | Partial | `desktop/tests/store.test.ts` (2 cases); `desktop/tests/activity-store.test.ts` — "writes no plaintext activity to disk", "refuses to store activity when encryption is unavailable"; `desktop/scripts/smoke-packaged.mjs` — "activity is encrypted at rest", "deleting activity erases it from disk, not just from the view" | Activity now gets the same refuse-rather-than-degrade treatment as chat, and the packaged smoke reads the real encrypted file while records exist rather than after they are deleted. Unit tests still inject a fake `safeStorage`, so real OS keychain behavior and the Linux `basic_text` backend check are exercised only by hand. |
...
| `[HC-NO-EXFILTRATION]` | Partial | `desktop/tests/engine/providers.test.ts` — "gives the runtime no ambient context to read"; `desktop/tests/engine/integrations/runner.test.ts` — "never touches an adapter that is turned off", "contacts only the hosts the constitution names"; `desktop/tests/engine/integrations/github.test.ts` — "contacts only api.github.com", "uses read-only requests", "sends only a query and a credential, never user content", "fetches nothing when no repository is in scope", "searches without a repository qualifier once all repositories are opted in", "bounds a first sync to the lookback window"; `desktop/tests/engine/integrations/notion.test.ts` — "contacts only api.notion.com", "sends only a filter and a cursor, never user content", "makes no request when no database is chosen", "refuses to query without a credential", "resumes from the last sync rather than the lookback window"; `desktop/tests/engine/integrations/google-sheets.test.ts` — "admits exactly the two hosts the constitution names", "refuses everything else, including neighbouring Google hosts", "contacts only the two hosts the constitution names", "sends an assertion and a range, never user content", "makes no request at all when no spreadsheet is chosen", "refuses to read without a stored key"; `desktop/scripts/smoke-packaged.mjs` — "activity is collected only after the user turns it on" | The Copilot SDK's ambient-file defaults are pinned, and both shipped adapters now automate the ingress-only half of the exemption: a host that matches the declaration, a request body carrying no user content, and no request at all when scope is empty. Those assertions are still written per adapter rather than enforced as a property over the registry — the registry-level tests check only the declared host list and the credential pairing, not what any adapter actually sends, so every new adapter has to remember to write its own and nothing fails if it does not. There is still no dependency policy or process-wide network assertion, so a new outbound call elsewhere in the codebase would pass the suite. |
...
| `[HC-CITATIONS-RESOLVE]` | Automated | `desktop/tests/engine/validation.test.ts` — "rejects an unknown citation", "accepts resolved attribution"; `desktop/tests/engine/activity-grounding.test.ts` — "rejects a response citing an activity ID that was not in the request", "rejects a cited signal when no activity was sent at all", "does not accept a real source ID in the activity lane" | The activity lane resolves one-directionally by design: a cited signal must exist in the request, but the model is not obliged to cite every signal it saw, because signals are context rather than support. |
...
| `[HC-OBSERVATION-VS-INFERENCE]` | Automated | `desktop/tests/engine/validation.test.ts` — "preserves observation and inference fields"; `desktop/tests/engine/activity-grounding.test.ts` — "keeps measured activity out of the user's own claims", "keeps signals out of the mentor's evidence lane", "carries measured activity alongside, not inside, the user's claims"; `desktop/tests/engine/integrations/rollup.test.ts` — "names the user's day, not UTC's, on a Denver evening", "names the user's day east of Greenwich too"; `desktop/tests/engine/activity-grounding.test.ts` — "says how many signals qualified when the cap truncates them", "says how many qualified when nothing was truncated", "counts only what qualified, not everything stored" | Field separation is enforced, and measured activity is now provably kept out of both `current_state` (what the user claims) and `sources` (what supports the mentor). Whether the model actually put the right content in each field is still not checked. "Today" is now the user's calendar day rather than UTC's, which for six hours every evening west of Greenwich were different days — the window an observation falls in is part of what is being observed. Truncation is now declared rather than silent: a capped signal list travels with the number that qualified, after a model shown twelve of thirty-six commits reported the twelve as the total. What the model does with that number is prompt guidance, which nothing here verifies. |
