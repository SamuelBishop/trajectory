...
| `[HC-OBSERVATION-VS-INFERENCE]` | Automated | `desktop/tests/engine/validation.test.ts` — "preserves observation and inference fields"; `desktop/tests/engine/activity-grounding.test.ts` — "keeps measured activity out of the user's own claims", "keeps signals out of the mentor's evidence lane", "carries measured activity alongside, not inside, the user's claims"; `desktop/tests/engine/integrations/rollup.test.ts` — "names the user's day, not UTC's, on a Denver evening", "names the user's day east of Greenwich too" | Field separation is enforced, and measured activity is now provably kept out of both `current_state` (what the user claims) and `sources` (what supports the mentor). Whether the model actually put the right content in each field is still not checked. "Today" is now the user's calendar day rather than UTC's, which for six hours every evening west of Greenwich were different days — the window an observation falls in is part of what is being observed. |
